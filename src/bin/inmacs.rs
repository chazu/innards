...
fn main() -> Result<()> {
    ...
    let mut app = Editor::open(config.path.clone(), config.line, config.height)?;
    let syntax = SyntaxHighlighter::new(&config.path)?;

    let mut terminal = TerminalGuard::enter(config.height)?;
    terminal
        .terminal
        .draw(|frame| draw(frame, &mut app, &syntax))?;
    let outcome = run_editor(&mut terminal, &mut app, &syntax)?;
    drop(terminal);
...
impl Config {
    fn parse(args: impl Iterator<Item = String>) -> Result<Self> {
        ...
        let path = path.ok_or_else(|| anyhow!("usage: inmacs [--height N] [+LINE] FILE"))?;
...
impl Editor {
    fn open(path: PathBuf, line: Option<usize>, height: u16) -> Result<Self> {
        ...
        Ok(Self {
            ...
            dirty: false,
            status: "Ctrl-S search  Ctrl-R reverse-search  Ctrl-X Ctrl-S save".to_string(),
...
fn handle_key(
    ...
    if app.ctrl_x_pending {
        return handle_ctrl_x_chord(app, key);
    }
...
fn handle_ctrl_x_chord(app: &mut Editor, key: KeyEvent) -> Result<Option<Outcome>> {
...
fn draw(frame: &mut Frame<'_>, app: &mut Editor, syntax: &SyntaxHighlighter) {
    ...
    let block = Block::default()
        .title(format!(" inmacs: {} ", app.path.display()))
        .borders(Borders::ALL);
